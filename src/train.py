...
from TAN import Full#our TAN
from MTSa import MaskedTemporalSelfAttention#ablation
from TDa import TDa#ablation
...
class Trainer:
    ...
    def train(self):
        ...
        if self.config.model == 'MTSa':
            model = MaskedTemporalSelfAttention(
            ...
            self.log_file = os.path.join(
                self.config.log_path,
                self.cmd + '_' + str(self.config.attn_wnd)
            )
            self.model_file = os.path.join(
                self.config.model_save_path,
                self.cmd + '_' + str(self.config.attn_wnd) + '.pt'
            )
        elif self.config.model == 'TAN':
            model = Full(
                self.config.input_dim,
                self.config.hidden_dim,
                self.config.output_dim,
